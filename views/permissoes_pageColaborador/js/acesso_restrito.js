...
function resolverCaminhoPagina(pagina) {
  const pathname = window.location.pathname.toLowerCase();
  const usaLoginPaciente = pathname.includes('/views/loginp/');
...
function redirecionarPorNivel(nivel) {
  if (nivel === 1) return window.location.href = resolverCaminhoPagina('n1_pagina.html');
  if (nivel === 2) return window.location.href = resolverCaminhoPagina('paciente.html');
  if (nivel === 3) return window.location.href = resolverCaminhoPagina('colaborador.html');
  return window.location.href = resolverCaminhoPagina('acesso_restrito.html');
}
...
function verificarAcessoPagina() {
  ...
  if (!usuario) {
    ...
    if (pagina !== 'login.html' && pagina !== 'home.html') {
      window.location.href = getLoginPageUrl();
    }
    return;
  }

  if (pagina === 'index.html' || pagina === 'login.html' || pagina === 'home.html') {
