...
<body class="bg-[#f8fafc] text-slate-800 font-sans antialiased min-h-screen">
  ...
  <div id="app-root" class="min-h-screen bg-[#f8fafc]">
    ...
    @if ($nivelUsuario === 1)
      <!-- ========================================================================= -->
      <!-- VISÃO DO PACIENTE (CARDS RESPONSIVOS)                                     -->
      <!-- ========================================================================= -->
      <main id="main-content" class="max-w-xl w-full mx-auto p-4 sm:p-6 space-y-5 pb-20">
        ...
        <div class="space-y-4">
          @forelse($showAgendamentos as $agendamento)
            @php
              $statusRaw = strtolower($agendamento->status_comparecimento ?? $agendamento->status_agendamento ?? $agendamento->status ?? 'agendado');

              if ($statusRaw === 'confirmado' || $statusRaw === 'presente' || $statusRaw === 'agendado') {
      ...
      <main id="main-content" class="max-w-7xl w-full mx-auto p-6 md:p-8 space-y-6">
        ...
        <form id="filterForm" method="GET" action="{{ url()->current() }}" class="bg-white rounded-xl border border-slate-100 p-4 md:p-6 flex flex-col md:flex-row items-center justify-between gap-4 shadow-xs">
          ...
          <div class="flex flex-wrap items-center gap-3 w-full md:w-auto justify-end">
            ...
            <!-- Filtro Status Agendamento / Comparecimento -->
            <select
              name="status"
              onchange="this.form.submit()"
              class="bg-white border border-slate-200 text-slate-600 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-sky-500 transition-all cursor-pointer">
  ...
  <script>
    ...
    function setSidebarExpanded(expanded) {
      ...
      if (sidebarOverlay) {
        ...
        sidebarOverlay.classList.toggle('opacity-100', expanded);
        sidebarOverlay.classList.toggle('opacity-0', !expanded);
      }
    ...
    function alternarAbaDoc(tipo) {
      ...
      if (!painelRg || !painelReq) return;

      if (tipo === 'rg') {
        painelRg.style.display = 'block';
        painelReq.style.display = 'none';
        if (btnRg) btnRg.className = 'px-3.5 py-1.5 rounded-lg text-xs font-bold bg-sky-600 text-white shadow-xs transition-all cursor-pointer';
        if (btnReq) btnReq.className = 'px-3.5 py-1.5 rounded-lg text-xs font-medium text-slate-600 hover:text-slate-900 transition-all cursor-pointer';
      } else {
        painelRg.style.display = 'none';
        painelReq.style.display = 'block';
        if (btnReq) btnReq.className = 'px-3.5 py-1.5 rounded-lg text-xs font-bold bg-sky-600 text-white shadow-xs transition-all cursor-pointer';
        if (btnRg) btnRg.className = 'px-3.5 py-1.5 rounded-lg text-xs font-medium text-slate-600 hover:text-slate-900 transition-all cursor-pointer';
      }
    }
    ...
    function renderizarDocumentoEmbutido(urlBruta, containerId, nomePadrao, tipoDoc, idAgendamento) {
      ...
      let urlFinal = urlBruta.trim();
      if (!urlFinal.startsWith('http://') && !urlFinal.startsWith('https://') && !urlFinal.startsWith('/')) {
        const partes = urlFinal.split('/');
        const nomeArquivo = partes[partes.length - 1];
        urlFinal = '/storage/documentos_agendamentos/' + nomeArquivo;
      }

      const isPdf = urlFinal.toLowerCase().includes('.pdf');

      if (isPdf) {
        container.innerHTML = `
          <div class="w-full space-y-2">
            <iframe src="${urlFinal}#toolbar=0" class="w-full h-80 rounded-lg border border-slate-200 bg-white" title="${nomePadrao}"></iframe>
            <div class="text-right">
              <a href="${urlFinal}" target="_blank" class="text-[11px] text-sky-600 hover:underline font-semibold">
        ...
        container.innerHTML = `
          <div class="w-full flex flex-col items-center justify-center p-3 bg-slate-900/5 rounded-xl border border-slate-200 overflow-hidden space-y-2">
            <img src="${urlFinal}" alt="${nomePadrao}" class="max-h-80 w-auto object-contain rounded-lg shadow-sm bg-white" />
            <div class="w-full flex justify-between items-center text-[11px] text-slate-500 px-1 pt-1">
              ...
              <a href="${urlFinal}" target="_blank" class="text-sky-600 hover:text-sky-800 hover:underline font-semibold flex items-center gap-1">
                Abrir imagem em tela cheia &nearr;
              </a>
            </div>
          </div>
        `;
      ...
      fetch('/agendamentos/' + id)
        ...
        .then(data => {
          if (data && data.agendamento) {
            ...
            document.getElementById('modalExame').innerText = a.nome_vaga || a.tipo_exame || a.vaga || 'Atendimento Geral';
