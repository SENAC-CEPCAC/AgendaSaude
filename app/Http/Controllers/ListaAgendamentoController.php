...
{
    /**
     * Exibe a listagem de agendamentos para Triagem Administrativa (N1).
    ...
    public function index(Request $request)
    {
        ...
        // Filtro de busca textual (Nome do paciente, CPF ou Nº do agendamento)
            ...
            $query_agendamentos->where(function ($query) use ($termo_busca, $cpf_limpo) {
                $query->where('id_prontuario', $termo_busca)
                    ->orWhereHas('paciente', function ($q_paciente) use ($termo_busca, $cpf_limpo) {
                        ...
                        if (!empty($cpf_limpo)) {
                            $q_paciente->orWhere('cpf', 'like', "%{$cpf_limpo}%");
                        }
                    });
            });
        }
    ...
    /**
     * Atualiza o status de comparecimento do agendamento (ex: confirmado, presente, faltou, cancelado).
     */
    public function atualizar_status(Request $request, $id_prontuario)
    {
        ...
        return redirect()->back()->with('sucesso', "Status do agendamento #{$id_prontuario} atualizado para '{$dados_validados['status_comparecimento']}'.");
    }
    ...
    public function avaliar_documento(Request $request, $id_prontuario)
    {
        ...
        $mensagem = $dados_validados['status_documento'] === 'aprovado'
            ? "Documento do agendamento #{$id_prontuario} aprovado com sucesso!"
            : "Documento do agendamento #{$id_prontuario} foi rejeitado. O paciente poderá reanexar um novo arquivo.";
    ...
    public function reanexar_documento(Request $request, $id_prontuario)
    {
        ...
        return redirect()->back()->with('sucesso', "Novo documento reanexado com sucesso para o agendamento #{$id_prontuario}!");
    }
}